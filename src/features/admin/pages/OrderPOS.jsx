    ...
    return (
        <div className="p-4 md:p-6 font-sans pb-24 md:pb-8 flex flex-col lg:flex-row gap-5 h-[calc(100vh-56px)] overflow-hidden">
            ...
            <div className="flex-1 flex flex-col gap-4 overflow-hidden min-w-0">
                ...
                {/* Chọn khách hàng */}
                <div ref={customerDropdownRef} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex-shrink-0 relative z-30">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        {selectedCustomer && (
                            <button onClick={() => setSelectedCustomer(null)} className="text-xs text-slate-500 hover:text-red-400 transition-colors flex items-center gap-1">
                                ...
                                <ul className="absolute z-40 w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden divide-y divide-slate-700/50 max-h-56 overflow-y-auto">
                                    {customers.map(c => (
                                        <li
                                            ...
                                            onMouseDown={() => { setSelectedCustomer(c); setSearchCustomer(''); setShowCustomerDropdown(false); }}
                                        >
                                            <div>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl flex flex-col flex-1 overflow-hidden">
                    <div className="px-4 pt-3 pb-0 border-b border-slate-800 flex flex-col gap-3 flex-shrink-0">
            ...
            <div className="lg:w-[340px] xl:w-[380px] bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden flex-shrink-0 relative">
                ...
                {/* Customer indicator */}
                {selectedCustomer && (
                    <div className="bg-emerald-500/5 border-b border-emerald-500/10 px-5 py-2 flex justify-between items-center flex-shrink-0">
                        <span className="text-[11px] text-slate-500 uppercase tracking-widest font-bold">Khách:</span>
                        <span className="text-xs font-black text-emerald-400 truncate max-w-[180px]">{selectedCustomer.name}</span>
                    </div>
                )}
